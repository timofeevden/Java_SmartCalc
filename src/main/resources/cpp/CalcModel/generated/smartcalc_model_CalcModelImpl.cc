#include "smartcalc_model_CalcModelImpl.h"
...
JNIEXPORT jstring JNICALL Java_smartcalc_model_CalcModelImpl_translateToPolish(
    JNIEnv* env, jobject javaObj, jstring javaExpression) {
  jboolean isCopy;
  const char* convertedValue = env->GetStringUTFChars(javaExpression, &isCopy);
  std::string cppExpression = convertedValue;

  if (isCopy == false) {
    return NULL;
  }
  s21::CalcModel* model = s21::CalcModel::GetInstance();
  model->TranslateToPolish(cppExpression);

  if (model->GetError() != 0) {
    return NULL;
  }
  std::string polish = model->GetPolishExpression();
  jstring result = env->NewStringUTF(polish.c_str());
  return result;
}
...
Java_smartcalc_model_CalcModelImpl_calculatePolishExpression(JNIEnv* env,
                                                             jobject javaObj,
                                                             jdouble javaX) {
  s21::CalcModel* model = s21::CalcModel::GetInstance();
  double cppX = javaX;
...
Java_smartcalc_model_CalcModelImpl_getErrorNumber(JNIEnv*, jobject) {
  s21::CalcModel* model = s21::CalcModel::GetInstance();
 ...
 */
JNIEXPORT jdouble JNICALL
Java_smartcalc_model_CalcModelImpl_getCalculatingResult(JNIEnv*, jobject) {
  s21::CalcModel* model = s21::CalcModel::GetInstance();
...
JNIEXPORT void JNICALL Java_smartcalc_model_CalcModelImpl_calculateGraphData(
    JNIEnv* env, jobject, jdouble xMin, jdouble xMax, jdouble yMin,
    jdouble yMax) {
  s21::CalcModel* model = s21::CalcModel::GetInstance();
  model->CalculateDataGraph(xMin, xMax, yMin, yMax);
}
...
Java_smartcalc_model_CalcModelImpl_getGraphDataFromDLL(JNIEnv* env, jobject) {
  jclass list_cls = env->FindClass("java/util/ArrayList");
  if (list_cls == NULL) {
    std::cout << "ArrayList no  find!\n\n";
    return NULL;
  }
  ...
  s21::CalcModel* model = s21::CalcModel::GetInstance();
  std::vector<double> data_x = model->GetDataX();
  std::vector<double> data_y = model->GetDataY();

  jboolean jReturnValue;
  for (int i = 0; i < data_x.size(); ++i) {
    jobject second_list = env->NewObject(list_cls, list_init, "");
    jobject x_obj = env->NewObject(
        env->FindClass("java/lang/Double"),
        env->GetMethodID(env->FindClass("java/lang/Double"), "<init>", "(D)V"),
        data_x[i]);
    jobject y_obj = env->NewObject(
        env->FindClass("java/lang/Double"),
        env->GetMethodID(env->FindClass("java/lang/Double"), "<init>", "(D)V"),
        data_y[i]);
    jReturnValue = env->CallBooleanMethod(second_list, list_add, x_obj);
    jReturnValue = env->CallBooleanMethod(second_list, list_add, y_obj);
    jReturnValue = env->CallBooleanMethod(list_obj, list_add, second_list);
  }
  return list_obj;
}
...
CalcModel* CalcModel::GetInstance() {
  if (s21::CalcModel::instance_ptr_ == nullptr) {
    s21::CalcModel::instance_ptr_ = new CalcModel();
  }
  return s21::CalcModel::instance_ptr_;
}
...
int CalcModel::TranslateToPolish(const std::string& new_expression) {
  ...
  while (stack_operators.empty() != true && !error_) {
    polish_expression_.push_back(stack_operators.top());
    stack_operators.pop();
  }

  if (polish_expression_.empty() && !error_) {
...
/*
//  Проверяем на число в выражении
bool CalcModel::CheckX(int& i) {
          if (expression_[i] == 'x' || expression_[i] == 'X') {
      polish_expression_.push_back('X');  //  число Х
    } else if (expression_[i] == 'P' && expression_[i + 1] == 'i') {
      polish_expression_.push_back('P');  //  число Pi
      ++i;
...
double CalcModel::TakeNumber(int& i) {
  ...
  while (IsNumber(polish_expression_[i]) || polish_expression_[i] == '.' ||
    ...
    if (polish_expression_[i] == '.') {
      if (was_dot != false) {
        error_ = 21;  //  лишняя точка
        break;
      } else {
        was_dot = true;
      }
...
void CalcModel::SetPolishExpression(const std::string& new_polish_expression) {
  polish_expression_ = new_polish_expression;
  expression_.clear();
}
